...
router.post('/growin/create', async (req, res) => {
    try {
        ...
        const paymentData = {
            pageCode: "076c48159335",
            userId: "2c731697f2abda19",
            cancelUrl: `http://localhost:3000/checkout/cancel?bookingId=${booking.id}`,
            successUrl: `http://localhost:3000/checkout/success?bookingId=${booking.id}`,
            // Add backend callback URLs for Grow to call
            notifyUrl: `https://0c9120a98090.ngrok-free.app/api/checkout/payment/success?bookingId=${booking.id}`,
        ...
        if (transactionData || (processId && growinConfirmationCode)) {
            try {
                ...
                const approveResponse = await axios.post(
                    'https://sandbox.meshulam.co.il/api/light/server/1.0/approveTransaction',
                    approveFormData,
                    {
